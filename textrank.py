...
# 关键短语抽取
def keyphrases_extraction(text, keywords_num, min_occur_num):
    tr4w = TextRank4Keyword()
    tr4w.analyze(text=text, window=2, lower=True, vertex_source='all_filters', edge_source='no_stop_words',
                 pagerank_config={'alpha': 0.85, })
    keyphrases = tr4w.get_keyphrases(keywords_num=keywords_num, min_occur_num=min_occur_num)
...
# 关键句抽取
def keysentences_extraction(text, num, sentence_min_len):
    ...
    keysentences = tr4s.get_key_sentences(num=num, sentence_min_len=sentence_min_len)
    return keysentences
...
if __name__ == "__main__":
    text = "来源：中国科学报本报讯（记者肖洁）又有一位中国科学家喜获小行星命名殊荣！4月19日下午，中国科学院国家天文台在京举行“周又元星”颁授仪式，" \
           ...
           "正式将该小行星命名为“周又元星”。"
    # textrank4zh
    print("使用textrank4zh的方法")
    # 关键词抽取
    keywords = keywords_extraction(text, num=4, word_min_len=1)
    print(keywords)
    # 关键短语抽取
    keyphrases = keyphrases_extraction(text, keywords_num=3, min_occur_num=2)
    print(keyphrases)
    # 关键句抽取
    keysentences = keysentences_extraction(text, num=3, sentence_min_len=5)
    print(keysentences)
